...
const Projects = () => {
  ...
  const handleAddToTask = (project, email) => {
    const projectInfo = { project, email };
    ...
    <div>
      ...
      <div className="w-full grid lg:grid-cols-3 gap-4">
        {projects.map((project, index) => {
          ...
          const userHasAccess = project.access.includes(currentUser?.email);
                  ...
                  {members?.map((member, i) => <li key={i} className="pb-3 sm:pb-4">
                    <div className="flex items-center space-x-4 rtl:space-x-reverse">
                      ...
                      <div className="inline-flex items-center text-base font-semibold text-gray-900">
                        <button onClick={() => handleAddToTask(project, member?.email)} className="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-blue-600 rounded-lg hover:bg-blue-500 focus:ring-4 focus:outline-none focus:ring-blue-300 ">Add</button>
                      </div>
                    </div>
                  </li>
                  )}
